...
class AnalysisProcessor(processor.ProcessorABC):
    ...
    def __init__(self, year, xsec, corrections, ids, common):
        ...
        self._samples = {
            'sre1':('WJets','DY','TT','ST','WW','WZ','ZZ','QCD','SingleElectron','SinglePhoton','EGamma'),
            'sre2':('WJets','DY','TT','ST','WW','WZ','ZZ','QCD','SingleElectron','SinglePhoton','EGamma'),
            'srm1':('WJets','DY','TT','ST','WW','WZ','ZZ','QCD','SingleMuon'),
            'srm2':('WJets','DY','TT','ST','WW','WZ','ZZ','QCD','SingleMuon'),
            'ttbare1':('WJets','DY','TT','ST','WW','WZ','ZZ','QCD','SingleElectron','SinglePhoton','EGamma'),
            'ttbare2':('WJets','DY','TT','ST','WW','WZ','ZZ','QCD','SingleElectron','SinglePhoton','EGamma'),
            'ttbarm1':('WJets','DY','TT','ST','WW','WZ','ZZ','QCD','SingleMuon'),
            'ttbarm2':('WJets','DY','TT','ST','WW','WZ','ZZ','QCD','SingleMuon'),
            'wjete1':('WJets','DY','TT','ST','WW','WZ','ZZ','QCD', 'SingleElectron','SinglePhoton','EGamma'),
            'wjete2':('WJets','DY','TT','ST','WW','WZ','ZZ','QCD', 'SingleElectron','SinglePhoton','EGamma'),
            'wjetm1':('WJets','DY','TT','ST','WW','WZ','ZZ','QCD','SingleMuon'),
            'wjetm2':('WJets','DY','TT','ST','WW','WZ','ZZ','QCD','SingleMuon'),
...
#         j_niso=j_iso.counts
        ...
        mT = {
            'sre1': np.sqrt(2*leading_e.pt.sum()*met.pt*(1-np.cos(met.T.delta_phi(leading_e.T.sum())))),
            'srm1': np.sqrt(2*leading_mu.pt.sum()*met.pt*(1-np.cos(met.T.delta_phi(leading_mu.T.sum())))),
            'ttbare1': np.sqrt(2*leading_e.pt.sum()*met.pt*(1-np.cos(met.T.delta_phi(leading_e.T.sum())))),
            'ttbarm1': np.sqrt(2*leading_mu.pt.sum()*met.pt*(1-np.cos(met.T.delta_phi(leading_mu.T.sum())))),
            'wjete1': np.sqrt(2*leading_e.pt.sum()*met.pt*(1-np.cos(met.T.delta_phi(leading_e.T.sum())))),
            'wjetm1': np.sqrt(2*leading_mu.pt.sum()*met.pt*(1-np.cos(met.T.delta_phi(leading_mu.T.sum())))),
          
            'sre2': np.sqrt(2*leading_e.pt.sum()*met.pt*(1-np.cos(met.T.delta_phi(leading_e.T.sum())))),
            'srm2': np.sqrt(2*leading_mu.pt.sum()*met.pt*(1-np.cos(met.T.delta_phi(leading_mu.T.sum())))),
            'ttbare2': np.sqrt(2*leading_e.pt.sum()*met.pt*(1-np.cos(met.T.delta_phi(leading_e.T.sum())))),
            'ttbarm2': np.sqrt(2*leading_mu.pt.sum()*met.pt*(1-np.cos(met.T.delta_phi(leading_mu.T.sum())))),
            'wjete2': np.sqrt(2*leading_e.pt.sum()*met.pt*(1-np.cos(met.T.delta_phi(leading_e.T.sum())))),
            'wjetm2': np.sqrt(2*leading_mu.pt.sum()*met.pt*(1-np.cos(met.T.delta_phi(leading_mu.T.sum()))))
        }
...
#             }
            trig = {
                'sre1': np.ones(events.size),
                'srm1':np.ones(events.size),
                'ttbare1': np.ones(events.size),
                'ttbarm1': np.ones(events.size),
                'wjete1': np.ones(events.size),
                'wjetm1':np.ones(events.size),
              
                'sre2': np.ones(events.size),
                'srm2':np.ones(events.size),
                'ttbare2': np.ones(events.size),
                'ttbarm2': np.ones(events.size),
                'wjete2': np.ones(events.size),
                'wjetm2':np.ones(events.size),
                #                 'dilepe' : get_met_trig_weight(leading_e.eta.sum(),leading_e.pt.sum()),
                #                 'dilepm' : get_met_trig_weight(leading_mu.eta.sum(),leading_mu.pt.sum()),
            }
            ###
            ...
            ids = {
                'sre1': get_ele_tight_id_sf(leading_e.eta.sum(), leading_e.pt.sum()),
                'srm1': get_mu_tight_id_sf(leading_mu.eta.sum(), leading_mu.pt.sum()),
                'ttbare1': get_ele_tight_id_sf(leading_e.eta.sum(), leading_e.pt.sum()),
                'ttbarm1': get_mu_tight_id_sf(leading_mu.eta.sum(), leading_mu.pt.sum()),
                'wjete1': get_ele_tight_id_sf(leading_e.eta.sum(), leading_e.pt.sum()),
                'wjetm1': get_mu_tight_id_sf(leading_mu.eta.sum(), leading_mu.pt.sum()),
                
                'sre2': get_ele_tight_id_sf(leading_e.eta.sum(), leading_e.pt.sum()),
                'srm2': get_mu_tight_id_sf(leading_mu.eta.sum(), leading_mu.pt.sum()),
                'ttbare2': get_ele_tight_id_sf(leading_e.eta.sum(), leading_e.pt.sum()),
                'ttbarm2': get_mu_tight_id_sf(leading_mu.eta.sum(), leading_mu.pt.sum()),
                'wjete2': get_ele_tight_id_sf(leading_e.eta.sum(), leading_e.pt.sum()),
                'wjetm2': get_mu_tight_id_sf(leading_mu.eta.sum(), leading_mu.pt.sum()),
            ...
            reco = {
                'sre1': sf(leading_e.eta.sum()+leading_e.deltaEtaSC.sum(), leading_e.pt.sum()),
                'srm1': np.ones(events.size),
                'ttbare1': sf(leading_e.eta.sum()+leading_e.deltaEtaSC.sum(), leading_e.pt.sum()),
                'ttbarm1': np.ones(events.size),
                'wjete1': sf(leading_e.eta.sum()+leading_e.deltaEtaSC.sum(), leading_e.pt.sum()),
                'wjetm1': np.ones(events.size),
              
                'sre2': sf(leading_e.eta.sum()+leading_e.deltaEtaSC.sum(), leading_e.pt.sum()),
                'srm2': np.ones(events.size),
                'ttbare2': sf(leading_e.eta.sum()+leading_e.deltaEtaSC.sum(), leading_e.pt.sum()),
                'ttbarm2': np.ones(events.size),
                'wjete2': sf(leading_e.eta.sum()+leading_e.deltaEtaSC.sum(), leading_e.pt.sum()),
                'wjetm2': np.ones(events.size),
            ...
            isolation = {
                'sre1': np.ones(events.size),
                'srm1': get_mu_tight_iso_sf(mueta, leading_mu.pt.sum()),
                'ttbare1': np.ones(events.size),
                'ttbarm1': get_mu_tight_iso_sf(mueta, leading_mu.pt.sum()),
                'wjete1': np.ones(events.size),
                'wjetm1': get_mu_tight_iso_sf(mueta, leading_mu.pt.sum()),
              
                'sre2': np.ones(events.size),
                'srm2': get_mu_tight_iso_sf(mueta, leading_mu.pt.sum()),
                'ttbare2': np.ones(events.size),
                'ttbarm2': get_mu_tight_iso_sf(mueta, leading_mu.pt.sum()),
                'wjete2': np.ones(events.size),
                'wjetm2': get_mu_tight_iso_sf(mueta, leading_mu.pt.sum()),
            ...
            btagUp = {}
            btagDown = {}
            btag['sre1'],   btagUp['sre1'],   btagDown['sre1'] = get_deepflav_weight['medium'](
                j_clean.pt, j_clean.eta, j_clean.hadronFlavour, '+1')
            btag['srm1'],   btagUp['srm1'],   btagDown['srm1'] = get_deepflav_weight['medium'](
                j_clean.pt, j_clean.eta, j_clean.hadronFlavour, '+1')
            btag['ttbare1'], btagUp['ttbare1'], btagDown['ttbare1'] = get_deepflav_weight['medium'](
                j_clean.pt, j_clean.eta, j_clean.hadronFlavour, '2')
            btag['ttbarm1'], btagUp['ttbarm1'], btagDown['ttbarm1'] = get_deepflav_weight['medium'](
                j_clean.pt, j_clean.eta, j_clean.hadronFlavour, '2')
            btag['wjete1'], btagUp['wjete1'], btagDown['wjete1'] = get_deepflav_weight['medium'](
                j_clean.pt, j_clean.eta, j_clean.hadronFlavour, '0')
            btag['wjetm1'], btagUp['wjetm1'], btagDown['wjetm1'] = get_deepflav_weight['medium'](
                j_clean.pt, j_clean.eta, j_clean.hadronFlavour, '0')
            
            btag['sre2'],   btagUp['sre2'],   btagDown['sre2'] = get_deepflav_weight['medium'](
                j_clean.pt, j_clean.eta, j_clean.hadronFlavour, '+1')
            btag['srm2'],   btagUp['srm2'],   btagDown['srm2'] = get_deepflav_weight['medium'](
                j_clean.pt, j_clean.eta, j_clean.hadronFlavour, '+1')
            btag['ttbare2'], btagUp['ttbare2'], btagDown['ttbare2'] = get_deepflav_weight['medium'](
                j_clean.pt, j_clean.eta, j_clean.hadronFlavour, '2')
            btag['ttbarm2'], btagUp['ttbarm2'], btagDown['ttbarm2'] = get_deepflav_weight['medium'](
                j_clean.pt, j_clean.eta, j_clean.hadronFlavour, '2')
            btag['wjete2'], btagUp['wjete2'], btagDown['wjete2'] = get_deepflav_weight['medium'](
                j_clean.pt, j_clean.eta, j_clean.hadronFlavour, '0')
            btag['wjetm2'], btagUp['wjetm2'], btagDown['wjetm2'] = get_deepflav_weight['medium'](
                j_clean.pt, j_clean.eta, j_clean.hadronFlavour, '0')  
...
#                       (j_clean[j_clean['btagDeepB'] > btagWP_medium].counts == 0))
        ...
        selection.add('DeltaR_LJ_Ele_mask', (DeltaR_LJ_Ele_mask))
        ...
        selection.add("dpi_ele<1.5", (abs(met['T'].delta_phi(leading_e['T'].sum()))) < 1.5)
        selection.add("dpi_ele>1.5", (abs(met['T'].delta_phi(leading_e['T'].sum()))) >= 1.5)
        selection.add("dpi_mu<1.5", (abs(met['T'].delta_phi(leading_mu['T'].sum()))) < 1.5)
        selection.add("dpi_mu>1.5", (abs(met['T'].delta_phi(leading_mu['T'].sum()))) >= 1.5)
...
#             selection.add(sel_name, select)
        regions = {
            'sre1': {'isoneE', 'exactly_1_medium_btag', 'noHEMj', 'met_filters', 'single_electron_triggers', 'exclude_low_WpT_JetHT',
                    'Delta_Phi_Met_LJ', 'DeltaR_LJ_Ele_mask', 'mt_sre1>40','met100','noHEMmet', "dpi_ele<1.5"},
            'srm1': {'isoneM', 'exactly_1_medium_btag', 'met_filters', 'single_muon_triggers', 'exclude_low_WpT_JetHT',
                    'Delta_Phi_Met_LJ', 'DeltaR_LJ_Mu_mask', 'mt_srm1>40',  'met100', 'noHEMmet','noHEMj', "dpi_mu<1.5"},
            'ttbare1': {'isoneE', 'atleast_2_medium_btag', 'noHEMj', 'met_filters', 'single_electron_triggers', 'exclude_low_WpT_JetHT',
                       'Delta_Phi_Met_LJ', 'DeltaR_LJ_Ele_mask', 'mt_ttbare1>40', 'met100', 'noHEMmet', "dpi_ele<1.5"},
            'ttbarm1': {'isoneM', 'atleast_2_medium_btag', 'met_filters', 'single_muon_triggers', 'exclude_low_WpT_JetHT',
                       'Delta_Phi_Met_LJ', 'DeltaR_LJ_Mu_mask', 'mt_ttbarm1>40' , 'met100','noHEMmet','noHEMj', "dpi_mu<1.5" },
            'wjete1': {'isoneE', 'zero_medium_btags', 'met_filters', 'single_electron_triggers', 'exclude_low_WpT_JetHT',
                      'Delta_Phi_Met_LJ', 'DeltaR_LJ_Ele_mask', 'mt_wjete1>40' , 'met100', 'leading_j>70', 'noHEMmet','noHEMj', "dpi_ele<1.5"},
            'wjetm1': {'isoneM', 'zero_medium_btags', 'met_filters', 'single_muon_triggers', 'exclude_low_WpT_JetHT',
                      'Delta_Phi_Met_LJ', 'DeltaR_LJ_Mu_mask', 'mt_wjetm1>40', 'met100' ,'noHEMj', 'leading_j>70','noHEMmet','noHEMj', "dpi_mu<1.5"},
          
            'sre2': {'isoneE', 'exactly_1_medium_btag', 'noHEMj', 'met_filters', 'single_electron_triggers', 'exclude_low_WpT_JetHT',
                    'Delta_Phi_Met_LJ', 'DeltaR_LJ_Ele_mask', 'mt_sre2>40','met100','noHEMmet', "dpi_ele>1.5"},
            'srm2': {'isoneM', 'exactly_1_medium_btag', 'met_filters', 'single_muon_triggers', 'exclude_low_WpT_JetHT',
                    'Delta_Phi_Met_LJ', 'DeltaR_LJ_Mu_mask', 'mt_srm2>40',  'met100', 'noHEMmet','noHEMj', "dpi_mu>1.5"},
            'ttbare2': {'isoneE', 'atleast_2_medium_btag', 'noHEMj', 'met_filters', 'single_electron_triggers', 'exclude_low_WpT_JetHT',
                       'Delta_Phi_Met_LJ', 'DeltaR_LJ_Ele_mask', 'mt_ttbare2>40', 'met100', 'noHEMmet',"dpi_ele>1.5"},
            'ttbarm2': {'isoneM', 'atleast_2_medium_btag', 'met_filters', 'single_muon_triggers', 'exclude_low_WpT_JetHT',
                       'Delta_Phi_Met_LJ', 'DeltaR_LJ_Mu_mask', 'mt_ttbarm2>40' , 'met100','noHEMmet','noHEMj', "dpi_mu>1.5"},
            'wjete2': {'isoneE', 'zero_medium_btags', 'met_filters', 'single_electron_triggers', 'exclude_low_WpT_JetHT',
                      'Delta_Phi_Met_LJ', 'DeltaR_LJ_Ele_mask', 'mt_wjete2>40' , 'met100', 'leading_j>70', 'noHEMmet','noHEMj', "dpi_ele>1.5"},
            'wjetm2': {'isoneM', 'zero_medium_btags', 'met_filters', 'single_muon_triggers', 'exclude_low_WpT_JetHT',
                      'Delta_Phi_Met_LJ', 'DeltaR_LJ_Mu_mask', 'mt_wjetm2>40', 'met100' ,'noHEMj', 'leading_j>70','noHEMmet','noHEMj', "dpi_mu>1.5"},
