...
for year in ['2016', '2017', '2018']:
    ...
    get_pu_weight[year] = lookup_tools.dense_lookup.dense_lookup(
        pu_hist.values, pu_hist.edges)
...
for year in ['2016', '2017', '2018']:
    ...
    get_met_trig_weight[year] = lookup_tools.dense_lookup.dense_lookup(
        met_trig_hist.values, met_trig_hist.edges)
...
for year in ['2016', '2017', '2018']:
    ...
    get_met_zmm_trig_weight[year] = lookup_tools.dense_lookup.dense_lookup(
        zmm_trig_hist.values, zmm_trig_hist.edges)
...
ele_trig_hists = {
    ...
    # monojet measurement for the combined trigger path
    '2017': uproot.open("data/trigger_eff/electron_trigger_sf_2017.root")['EGamma_SF2D'],
    # approved by egamma group: https://indico.cern.ch/event/924522/
    '2018': uproot.open("data/trigger_eff/electron_trigger_sf_2018.root")['EGamma_SF2D']
}
get_ele_trig_weight = {}
for year in ['2016', '2017', '2018']:
...
for year in ['2016', '2017', '2018']:
    ...
    get_pho_trig_weight[year] = lookup_tools.dense_lookup.dense_lookup(
        pho_trig_hist.values, pho_trig_hist.edges)
...
for year in ['2016', '2017', '2018']:
    ...
    get_ele_loose_id_sf[year] = lookup_tools.dense_lookup.dense_lookup(
        ele_loose_sf_hist.values, ele_loose_sf_hist.edges)
    ele_tight_sf_hist = ele_tight_files[year]["EGamma_SF2D"]
    get_ele_tight_id_sf[year] = lookup_tools.dense_lookup.dense_lookup(
        ele_tight_sf_hist.values, ele_tight_sf_hist.edges)
...
for year in ['2016', '2017', '2018']:
    ...
    get_ele_reco_sf[year] = lookup_tools.dense_lookup.dense_lookup(
        ele_reco_hist.values, ele_reco_hist.edges)
# 2017 has a separate set of weights for low pt electrons (pt<20).
ele_reco_lowet_hist = uproot.open(
    "data/ScaleFactor/2017_egammaEffi_txt_EGM2D_runBCDEF_passingRECO_lowEt.root")['EGamma_SF2D']
get_ele_reco_lowet_sf = lookup_tools.dense_lookup.dense_lookup(
    ele_reco_lowet_hist.values, ele_reco_lowet_hist.edges)
...
for year in ['2016', '2017', '2018']:
    ...
    get_pho_tight_id_sf[year] = lookup_tools.dense_lookup.dense_lookup(
        pho_tight_hist.values, pho_tight_hist.edges)
...
for year in ['2016', '2017', '2018']:
    ...
    get_pho_csev_sf[year] = lookup_tools.dense_lookup.dense_lookup(
        pho_csev_hist.values, pho_csev_hist.edges)
...
get_mu_loose_id_sf = {}
for year in ['2016', '2017', '2018']:
    get_mu_tight_id_sf[year] = lookup_tools.dense_lookup.dense_lookup(
        mu_tight_hist[year].values, mu_tight_hist[year].edges)
    get_mu_loose_id_sf[year] = lookup_tools.dense_lookup.dense_lookup(
        mu_loose_hist[year].values, mu_loose_hist[year].edges)
...
get_mu_loose_iso_sf = {}
for year in ['2016', '2017', '2018']:
    get_mu_tight_iso_sf[year] = lookup_tools.dense_lookup.dense_lookup(
        mu_iso_tight_hist[year].values, mu_iso_tight_hist[year].edges)
    get_mu_loose_iso_sf[year] = lookup_tools.dense_lookup.dense_lookup(
        mu_iso_loose_hist[year].values, mu_iso_loose_hist[year].edges)
...
for year in ['2016', '2017', '2018']:
    ...
    for p in ['dy', 'w', 'z', 'a']:
        ...
        for cv in correlated_variations:
            ...
            get_nnlo_nlo_weight[year][p][cv] = lookup_tools.dense_lookup.dense_lookup(
                hist.values, hist.edges)
        for uv in uncorrelated_variations[p]:
            hist = nnlo_file[p][histname[p]+uncorrelated_variations[p][uv]]
            get_nnlo_nlo_weight[year][p][uv] = lookup_tools.dense_lookup.dense_lookup(
                hist.values, hist.edges)
...
def get_msd_weight(pt, eta):
    ...
    cpar = np.array([1.09302, -0.000150068, 3.44866e-07, -
                     2.68100e-10, 8.67440e-14, -1.00114e-17])
    fpar = np.array([1.27212, -0.000571640, 8.37289e-07, -
                     5.20433e-10, 1.45375e-13, -1.50389e-17])
...
def get_ecal_bad_calib(run_number, lumi_number, event_number, year, dataset):
    ...
    regular_dataset = ""
    regular_dataset = [name for name in ["MET", "SinglePhoton",
                                         "SingleElectron", "EGamma"] if (name in dataset)]
...
class BTagCorrector:
    ...
    def btag_weight(self, pt, eta, flavor, tag):
        ...
        if '-1' in tag:
            ...
            nom = (1 - zerotag(eff_data_nom) - onetag(eff_data_nom)) / \
                (1 - zerotag(eff) - onetag(eff))
            up = (1 - zerotag(eff_data_up) - onetag(eff_data_up)) / \
                (1 - zerotag(eff) - onetag(eff))
            down = (1 - zerotag(eff_data_down) - onetag(eff_data_down)) / \
                (1 - zerotag(eff) - onetag(eff))
        elif '+1' in tag:
